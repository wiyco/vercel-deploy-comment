...
>
> If the job fails or is cancelled while current-run rows are still `In Progress`, a post cleanup marks only those current deployment rows as `Failed` or `Cancelled`.
>
